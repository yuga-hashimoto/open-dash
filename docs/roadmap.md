...
> **Current release gate (2026-07-14):** The [smart-speaker product audit](smart-speaker-audit.md)
> is the source of truth for the difference between code-complete, opt-in/fallback,
> and proven on a physical tablet. A checked item does not close its real-device
> gate unless the dated smoke-run evidence exists.
...
- **P14.1 offline STT**: **full Kotlin stack shipped** — whisper.cpp v1.8.4
  submodule + active CMake wire-up + JNI bridge (`WhisperCppBridge`) +
  ...
  Remaining: real-device accuracy, endpoint, latency, and thermal smoke.
- **P14.2 VAD**: silence-timeout + min-speech knobs split; amplitude VAD and
  the Silero ONNX auto-upgrade path are shipped. Live-room quality remains a
...
- **P14.8 power/thermal**: BatteryMonitor + ThermalMonitor with ambient UI chips
  and VoiceService gating done. Privacy-safe in-memory voice/power measurement
  instrumentation and redacted logcat export are now shipped; physical idle
  wattage measurement is still a device gate.
- **P14.9 neural TTS**: **Kotlin stack shipped; native inference open** — piper v1.2.0
  ...
  active-voice preference. Remaining: Android cross-compilation for
  piper-phonemize + espeak-ng + ONNX Runtime, then native playback validation.
...
- [ ] P14.8: Power/thermal profile — **battery saver + thermal throttle + saver UI indicator done**: BatteryMonitor + ThermalMonitor @Singletons; BATTERY_SAVER_ENABLED preference gates both; VoiceService skips wake word on low battery OR WARM/HOT thermal state. `SaverStateProvider` folds preference + battery + thermal into a single reactive `SaverState(active, reason)` (battery wins over thermal so the "plug in the device" message is the actionable one). AmbientScreen renders a `BatterySaver`-icon chip with the reason string when the saver is actively throttling — users see *why* wake-word went quiet. Privacy-safe `VoiceMeasurementRecorder` captures bounded wake/STT/TTS/latency/battery/thermal counters and exports a redacted report for the smoke run. Tests cover recorder bounds, redaction, and percentile summaries. Physical idle wattage measurement still TODO
- [ ] P14.9: Neural TTS option — **Kotlin stack shipped; native inference open**. Pipeline: piper v1.2.0 submodule → `PiperCppBridge` Kotlin wrapper (native methods declared, graceful fallback) → `PiperTtsProvider` → `AudioTrackPcmPlayer` → `PiperVoiceDownloader` with Range-resume on paired `.onnx` + `.onnx.json` files → `PiperVoiceCatalog` → `PiperVoicesCard` Settings UI. Any gate failure transparently falls back to `AndroidTtsProvider` so users always hear something. The remaining work is a separate Android port of piper-phonemize/espeak-ng/ONNX Runtime; see [phase-16-native-plan.md](phase-16-native-plan.md).
...
- [ ] P16.5: Local knowledge base — the bundled original starter corpus and
  SQLite FTS5 index now provide an offline fallback, with deterministic Kotlin
  fallback when the device SQLite build lacks FTS5. The corpus is intentionally
  bounded and is not a complete Wikipedia mirror; expansion and real-device
  retrieval-quality validation remain open. Personal FAQ persistence is P22.14.
...
- [x] P19.1 (Priority 2): JS-Sandbox Skill Runtime — real engine landed this cycle.
  ...
  still not done at the time**: the `call_tool`/`read_memory` tool API bridge.

  **`read_memory` shipped as a follow-up this cycle.** Real bridge, not a stub: SKILL.md
  gets a new optional `memory_keys` frontmatter field (comma-separated); `SkillParser`
  parses it into `Skill.memoryKeys`, `SkillToolExecutor.executeRunScript` pre-fetches
  exactly those keys from `MemoryDao` (never the whole store, never a key the skill
  didn't declare, and a missing key is just absent rather than an error) into
  `SkillScriptContext.memory`, and `SkillScriptWrapper` bakes that map into the wrapped
  JS source as an escaped object literal with a `read_memory(key)` function defined over
  it. This works despite bare `QuickJs.evaluate()` being fully synchronous with no live
  host-callback capability, because a *read* only needs the data to already exist in the
  script's scope before it runs — there's nothing to call back into.

  **`call_tool` is still not done, and not a simple follow-up** — investigated the actual
  constraint rather than re-asserting the earlier guess: cashapp/zipline's low-level
  `QuickJs` class genuinely has no way to register a native callback (`initOutboundChannel`/
  `getInboundChannel` are `internal` to the zipline module, reachable only from the
  higher-level `Zipline`/`ZiplineLoader` API built for precompiled Kotlin/JS bundles, not
  raw script text). A real `call_tool` needs either a JNI extension straight to QuickJS's
  C API (bypassing Zipline's abstraction — a much bigger, separate undertaking) or a
  multi-step continuation protocol (script requests one tool call via a sentinel return
  value, host executes it, host re-runs the *entire* script from scratch with the result
  injected — since QuickJS holds no state across separate `evaluate()` calls — meaning
  skill authors would have to write scripts in an idempotent/replayable style). Left as a
  dedicated follow-up given the design work involved.

  Also not done: real-device validation — wrote `QuickJsSkillScriptRuntimeE2ETest`
  (timeout/interrupt, object marshaling, sandboxing, syntax-error handling, `read_memory`
  hit/miss) but no emulator/device was available to run it in this environment; unit
  tests only cover `SkillScriptWrapper`'s pure string logic since `QuickJs.create()`
  needs an Android-ABI `.so` a host JVM can't load. Ref: gallery (google-ai-edge),
  openclaw (official TS repo), cashapp/zipline
...
- [x] P21.5 (Priority 1, implementation complete; physical-device validation remains): Wake-word-free `stop` / `cancel` / `snooze` during alarm/timer ringing. `AlertSessionCoordinator` opens a bounded STT capture only while a timer or in-app alarm is actively ringing; `AlertCommandRouter` handles Japanese/English unqualified commands without an id, preserves explicit id/label fast paths, refuses to choose arbitrarily when multiple alerts ring, and returns to wake-word listening after resolution, no-match budget, or failure. Alarm/timer managers expose ringing-state callbacks so the session survives wake-word detector recreation. Focused JVM tests cover routing, ambiguity, execution, state policy, and coordinator behavior. Real-device acoustic, locked-screen, and OEM mic/FGS validation remains a P22.1 / [real-device-smoke-test.md](real-device-smoke-test.md) gate.
...
- [x] P21.7 (Priority 1, user-approved dependency addition; **implementation wired into VoiceService;
  detection accuracy still needs a real device**): openWakeWord as an opt-in alternative
  wake-word engine. Vosk's always-on ASR-based keyword spotting is comparatively power-heavy;
  openWakeWord runs a small ONNX pipeline (melspectrogram → shared speech-embedding →
  per-keyword classifier) instead. New `com.microsoft.onnxruntime:onnxruntime-android:1.22.0`
  dependency (verified non-conflicting with voicevoxcore's bundled custom
  `libvoicevox_onnxruntime.so` — different Java package, different `.so` name, confirmed by a
  clean `assembleStandardDebug`). `OpenWakeWordModelCatalog` pins the three real v0.5.1 release
  ONNX files (melspectrogram/embedding/`hey_jarvis_v0.1`) by URL + sha256 + exact byte size —
  verified by actually downloading them and inspecting tensor shapes with `onnxruntime` in
  Python during implementation, not guessed. `OpenWakeWordModelDownloader` mirrors
  `WhisperModelDownloader`'s proven Range-resume shape for the three files.
  `OpenWakeWordFeatureExtractor` ports the upstream Python `AudioFeatures` streaming buffer
  algorithm — **cross-validated by running the actual `pip install openwakeword` reference
  package against the real downloaded model files and confirming matching buffer growth**.
  `OpenWakeWordDetector` implements `WakeWordDetector`, same interface `VoskWakeWordDetector`
  does. Only "hey jarvis" is offered — English-only fixed preset, unlike Vosk's arbitrary
  custom/Japanese keyword — so this is strictly additive/opt-in, never a silent replacement.

  **Wired into `VoiceService` this cycle** — re-checked the premise blocking this ("VoiceService
  hardcodes the concrete `VoskWakeWordDetector` type") and it held up (confirmed the field really
  was typed `VoskWakeWordDetector?`), but `grep`-ing every call site showed `VoiceService` only
  ever calls the two methods `WakeWordDetector` already declares (`start`/`stop`) — no
  Vosk-specific surface leaking through, and no `pause()`/`resume()` calls despite what an
  earlier session's own note claimed (that's dead code on `VoskWakeWordDetector` unrelated to
  this path). So the fix was genuinely mechanical: widened the field to the `WakeWordDetector`
  interface and added `WAKE_WORD_ENGINE` preference-gated selection (default `"vosk"`, unchanged
  for every existing user unless they explicitly opt in via the new `WakeWordEngineCard` in
  Settings). One real behavioral risk found and handled: `OpenWakeWordDetector.start()` fails
  *asynchronously* (5 retries with backoff, ~15s worst case, then gives up silently — no
  exception, no callback) rather than throwing synchronously, so `VoiceService`'s existing
  try/catch around detector construction can't see that failure. Added a 20s watcher
  (`watchForOpenWakeWordGiveUp`) that checks `isListening` after the retry budget is certainly
  exhausted and rebuilds with Vosk if it's still false — so a corrupt openWakeWord model or a
  failed ONNX session load can't leave the device with *no* working wake word, even for users who
  opted in. `WakeWordEngineCard` + `WakeWordEngineViewModel` (`ui/settings/wakeword/`) handle the
  3-file download + engine picker, with an explicit warning that switching only recognizes "hey
  jarvis" in English.

  **Explicitly NOT done**: real-device validation that the fallback watcher's 20s timing feels
  right in practice, and — the part that was always going to need a device regardless of any
  wiring — whether openWakeWord actually reliably detects "hey jarvis" against real room
  acoustics/accents, which no amount of code review can verify. `VoiceService` has no test
  coverage at all (pre-existing — it's a deeply Android-`Service`-coupled class this project
  doesn't have Robolectric for), so this change is unverified by any automated test; correctness
  here rests on careful manual review of the diff (documented above) rather than a red/green
  test cycle.
...
## Phase 22 — Priority 1: Smart-speaker release gate

The 2026-07-14 audit found that the project has broad implementation coverage but
does not yet have the evidence or recovery behavior required for a dependable
speaker. Keep these separate from feature-completion checkboxes: an implementation
can be complete while its physical-device gate is still open.

- [ ] P22.1: Physical-device release run — execute the dated [real-device smoke test](real-device-smoke-test.md) on a representative tablet, including ten wake attempts, fast-path audio, barge-in, offline/online recovery, reboot alarm, locked-screen behavior, 30-minute stability, and power/thermal notes.
- [x] P22.2: Wake-word self-healing — `VoiceService` observes Vosk/openWakeWord health, restarts detectors with bounded backoff, and exposes degraded state. Physical microphone/OEM recovery remains P22.1/P22.5 validation.
- [x] P22.3: Ambient degraded-mode action — ambient Home exposes a one-tap Talk action when hotword is disabled, unavailable, or failed; physical reachability remains unverified.
- [x] P22.4: STT error contract — no-match/timeouts stay quiet while microphone, permission, recognizer, and network failures receive actionable recovery copy and return to Idle; physical recognition behavior remains unverified.
- [ ] P22.5: Barge-in and background entry-point validation — measure interruption latency and verify boot, locked-screen, Quick Settings, media-button, assistant-session, and foreground entry points across supported Android/OEM combinations.
- [x] P22.6: Offline profile truth (diagnostic) — pure `OfflineVoiceProfile` evaluator + Voice Health `offline_voice_profile` item report component gates (local STT / local LLM / system TTS / neural TTS) and modes FullyOffline / LocalSTTAndLLMSystemTts / NotReady. **Still open:** physical validation of Whisper accuracy/latency; Piper native port (P14.9/P16.3); Android TTS language-pack offline confirmation on device.
- [x] P22.7: Remote-data disclosure — `RemoteDataPolicy` blocks the first remote voice turn until the localized API/cloud description has been acknowledged, and Settings exposes a persistent local-only switch. Physical first-use UX remains unverified.
- [ ] P22.8: Idle power profile — a bounded recorder and ordered acceptance run now capture observed battery/thermal data and explicit operator verdicts; measure always-on wake-word drain and document the target device result with external/device power evidence. Battery percentage alone is not wattage.
- [ ] P22.9: Smart-home runtime wiring — saved HA/SwitchBot/MQTT settings now reach the live Hilt clients; discovery starts from the service-owned voice path; SwitchBot native IDs, capability validation, provider read-back, and provider tests are covered. Matter has an injected `MatterCommandDispatcher` boundary with a default unavailable implementation (fail-closed, no optimistic state mutation; fake dispatcher for JVM tests). Remaining: real native Matter cluster SDK wiring and concrete provider acceptance on hardware.
- [x] P22.10: Command truthfulness — fast-path confirmations require a non-failing, confirmed result; provider read-back failures are surfaced as accepted-but-unconfirmed instead of success.
- [x] P22.11: Boot policy and device readiness — application/service-owned provider and device readiness gates are implemented. `BootPolicy` (pure decision object) + `BootReceiver` no longer start the microphone `VoiceService` FGS from `BOOT_COMPLETED` (illegal for targetSdk 35 / Android 14+); boot still runs `BootRescheduler` for alarms/reminders/scheduled routines. Always-on mic resumes when the user opens the app (`MainActivity` starts `VoiceService`). Physical reboot smoke of reschedule + post-launch wake-word remains P22.1 / [real-device-smoke-test.md](real-device-smoke-test.md).
- [x] P22.12: Provider readiness contract — `ProviderManager` initializes from app/service lifecycle, checks actual provider availability, and `VoiceService` waits for readiness or speaks a degraded state. Cold-boot physical validation remains P22.1/P22.5.
- [x] P22.13: Voice command authorization — capability validation is centralized; lock/unlock requires explicit voice confirmation, and success speech is suppressed until provider read-back confirms state. Physical authorization/E2E validation remains open.
- [x] P22.14: Durable personal knowledge — `KnowledgeToolExecutor` now uses a Room-backed `KnowledgeStore` with schema migration, tags serialization, add/search/remove persistence, and JVM coverage. Bundled Wikipedia-lite remains P16.5.
- [x] P22.15: Voice/power measurement instrumentation — bounded privacy-safe `VoiceMeasurementRecorder`, service/pipeline hooks, Voice Health summary, clear/export actions, and a redacted real-device smoke-run procedure. This records observed battery percent/thermal state and latency; it does not claim wattage or physical pass criteria.
- [x] P22.16: Matter command boundary — `MatterDeviceProvider` now delegates to an injected native-cluster dispatcher and fails closed when unavailable or unconfirmed; local state is never mutated on dispatch failure. Native SDK wiring and physical Matter acceptance remain open.
- [x] P22.17: Physical acceptance tooling — pure ordered `VoiceAcceptanceRun`, service actions for start/observe/verify/export/clear, and a dated smoke-run artifact format. The tooling records evidence but does not self-certify hardware behavior.
