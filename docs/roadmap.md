...
- [ ] P14.1: Offline STT provider — **full Kotlin stack shipped, dormant pending externalNativeBuild**. Pipeline: whisper.cpp v1.8.4 submodule → CMake `whisper_jni` target (guarded, built only when submodule present) → `WhisperCppBridge` Kotlin wrapper with lazy `System.loadLibrary` + UnsatisfiedLinkError fallback → `WhisperSttProvider` implementing the SpeechToText contract with per-utterance language + translate preference lookup → `AudioRecordPcmSource` backed by AudioRecord at 16 kHz + amplitude VAD for endpoint detection → `WhisperModelDownloader` with Range-resume MockWebServer-tested + `WhisperModelCatalog` (tiny / base / small q5_1) → `WhisperModelsCard` Settings UI with model download / delete / active-radio + language picker (auto / en / ja) + translate-to-English toggle → `OfflineStackCard` diagnostic. `SttModule` binds the real `WhisperSttProvider` when `WhisperCppBridge.isAvailable()` returns true and falls back to `OfflineSttStub` otherwise. On stock builds `externalNativeBuild` is commented out in `app/build.gradle.kts`, so the fallback ships today — re-enabling native builds flips everything on without code changes. Ref: whisper.cpp, ggerganov/whisper.cpp
- [ ] P14.2: VAD / endpoint detection — **amplitude VAD shipped** for the Whisper capture path (`AmplitudeVad` + 50 ms chunking in `AudioRecordPcmSource`) + `MIN_SPEECH_MS` / `SILENCE_TIMEOUT_MS` slider under Voice Interaction; AndroidSttProvider wires both into `EXTRA_SPEECH_INPUT_*`. Silero-ONNX VAD (P16) is the follow-up for whisper-quality voice activity detection. Ref: sherpa-onnx silero-vad binding
...
- [ ] P20.1 (Priority 1): Shopping/To-do lists — structured CRUD distinct from generic `remember` memory. New `ListToolExecutor`: `add_list_item(list_name, item)`, `remove_list_item`, `complete_item`, `list_items(list_name)`, `clear_list`. Multiple named lists (default "shopping", "todo", user-created). Room-backed (`ShoppingListEntity`/`ShoppingListDao`), survives restart. Fast-path matcher for "add X to my shopping list" / "買い物リストに追加して".
- [ ] P20.2 (Priority 1): Reminders — time-triggered spoken/notification reminder distinct from calendar events ("remind me in 20 minutes to take the trash out"). `ReminderToolExecutor`: `set_reminder(text, trigger_time)`, `list_reminders`, `cancel_reminder`. `AlarmManager`-backed (exact alarm, survives app-kill) + Room persistence; fires a notification + TTS announcement when due.
- [ ] P20.3 (Priority 1): In-app recurring alarms — parallel system to the existing `TimerManager`, replacing the current system-Clock-app-intent-only `set_alarm` (which can't be listed/canceled/snoozed by the app afterward — asymmetric with timers). New `AlarmManager`-backed store with Room persistence, day-of-week repeat, snooze, own ringtone/TTS trigger. `AlarmToolExecutor`: `set_alarm(time, repeat_days, label)`, `list_alarms`, `cancel_alarm`, `snooze_alarm`.
- [ ] P20.4 (Priority 2): Scheduled/recurring routines — extend the existing `Routine` model with an optional time-of-day + days-of-week trigger; a `WorkManager`-backed scheduler invokes `run_routine` automatically ("every morning at 7am run my morning routine"). No API for this today (Routines are manual-invoke only).
- [ ] P20.5 (Priority 1): Jokes / trivia / fun facts — bundled local dataset (JSON asset under `app/src/main/assets/`, same pattern as `BundledNewsFeeds`), no external API/key. New `FunToolExecutor`: `tell_joke`, `get_trivia`, `fun_fact`. Fast-path matcher for "tell me a joke" / "笑える話をして".
- [ ] P20.6 (Priority 1): Fast-path latency parity — wire the already-implemented `convert_units` and `calculate` tools into `FastPathRouter` (currently LLM-only despite being simple/deterministic), matching the <200ms budget the rest of "core" tools get.
- [ ] P20.7 (Priority 2, external service — flag for approval before starting): Translation — not implemented at all today. Candidate: on-device Google ML Kit Translation (offline models after first download, no API key, no per-request network call — fits this project's local-first bias better than a cloud translate API) but is a new Gradle dependency, needs explicit approval per CLAUDE.md.
- [ ] P20.8 (Priority 2, external service — flag for approval before starting): Sports scores / stock prices / traffic — none implemented (falls through to generic `web_search` today). Needs a per-domain free-tier API (sports: usually keyless public score APIs exist; stocks/traffic typically require an API key) — confirm acceptable providers before implementing.
- [ ] P20.9 (Priority 2, external service — flag for approval before starting, likely descoped): Music-streaming deep integration (Spotify Web API/Connect) — today only generic `ACTION_MEDIA_PLAY_FROM_SEARCH` intent + media-key dispatch, no OAuth, no direct track/playlist control. OAuth device/PKCE flow on a kiosk-style tablet is nontrivial; confirm scope and provider before starting.
