...
- [ ] P21.6 (Priority 2, deferred — needs approval + real-device validation): whisper.cpp `externalNativeBuild` re-enablement for offline STT. The Kotlin stack has been fully shipped and dormant since P14.1; flipping the build flag needs a working NDK/CMake toolchain to verify compilation succeeds, which this environment can't confirm, plus it's exactly the kind of native-toolchain change the project's own docs call out as needing careful prototyping (see P14.9's piper note for the same class of risk).
